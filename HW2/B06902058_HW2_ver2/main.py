                ...
                while not que.empty():
                    ...
                    for k in range(4):
                        ...
                        if arr[nx][ny] == 255 and visited[nx][ny] == 0:
                            ...
                            visited[nx][ny] = 1
                if loop_cnt >= 500:
                    draw.rectangle([mnx, mny, mxx, mxy], outline = 'red', width=3)
                    avg_x /= loop_cnt
                    avg_y /= loop_cnt
                    draw.ellipse([int(avg_x)-3, int(avg_y)-3, int(avg_x)+3, int(avg_y)+3], outline='red', fill='blue')
    output.save("prob_C_white.bmp")
